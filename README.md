...
A multi-agent platform that generates comprehensive blockchain and DeFi protocol analysis reports. The platform uses a pipeline of AI agents to gather, curate, and synthesize real-time on-chain data about any decentralized protocol.

## Features

- **On-Chain Data Retrieval**: Integrates with blockchain data sources to fetch live on-chain metrics, token data, and protocol statistics
- **Tokenomics Analysis**: Provides detailed breakdowns of token distribution, utility, and risk assessment
- **DeFi Protocol Expertise**: Analyzes Total Value Locked (TVL), governance activities, and protocol performance
- **Multi-Source Research**: Gathers data from various sources including protocol websites, DefiLlama, Etherscan, and more
- **AI-Powered Content Filtering**: Uses Tavily's relevance scoring for content curation
- **Real-Time Progress Streaming**: Uses WebSocket connections to stream analysis progress and results
...
1. **Research Nodes**:
   - `CompanyAnalyzer`: Researches core protocol information
   - `IndustryAnalyzer`: Analyzes market position and trends
   - `BlockchainAnalyzer`: Fetches and analyzes on-chain data
   - `TokenomicsAnalyzer`: Provides token economics analysis
