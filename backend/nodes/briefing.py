...
"""
        ...
        try:
            logger.info(f"Sending prompt to LLM for {category} category")
            logger.debug(f"Prompt length: {len(prompt)} characters")
            
            # Set safety settings to allow more output
            safety_settings = [
                {
            ...
            logger.info(f"Received response from LLM for {category} briefing (length: {len(content)})")
            ...
            logger.error(f"Error generating {category} briefing: {e}", exc_info=True)
            # Try a simple fallback prompt for this category
            try:
                logger.info(f"Attempting fallback prompt for {category}")
                fallback_prompt = f"Create a brief summary about {company} related to {category}."
                response = self.gemini_model.generate_content(fallback_prompt)
                content = response.text.strip()
                if content:
                    logger.info(f"Fallback successful for {category}")
                    return {'content': content}
