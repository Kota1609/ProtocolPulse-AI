...
class Editor:
    ...
    async def compile_content(self, state: ResearchState, briefings: Dict[str, str], company: str) -> str:
        ...
        prompt = f"""You are compiling a comprehensive research report about {company}, a blockchain protocol in the {industry} sector operating on the {hq_location} blockchain.

Compiled briefings:
{combined_content}

Create a detailed and focused protocol analysis report that:
1. Integrates information from all sections into a cohesive, non-repetitive narrative
2. Maintains important technical details and tokenomics information
3. Organizes information logically with a focus on protocol mechanics, ecosystem position, and on-chain metrics
4. Uses clear section headers and subsection structure

Formatting rules:
Strictly enforce this EXACT document structure:

# {company} Protocol Analysis

## Protocol Overview
- Core functionality and purpose
- Technical architecture
- Protocol history and major upgrades
- Governance mechanism

## Tokenomics
- Token utility and value accrual
- Distribution and supply metrics
...
Return the report in clean markdown format with detailed content in each section. Each section should have clearly marked subsections."""
        ...
        try:
            response = await self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system", 
                        "content": "You are an expert DeFi researcher that compiles comprehensive blockchain protocol reports. Focus on specific protocol mechanics, tokenomics, on-chain metrics, and ecosystem positioning."
                    },
                    {
                        "role": "user",
                        "content": prompt
    ...
    async def content_sweep(self, state: ResearchState, content: str, company: str) -> str:
        ...
        prompt = f"""You are an expert DeFi protocol analyst. You are given a report on {company}, a blockchain protocol in the {industry} sector operating on the {hq_location} blockchain.

Current report:
{content}

Refine this report to:
1. Remove redundant or repetitive information
2. Enhance technical accuracy and precision
3. Enrich sections lacking substantial on-chain or tokenomics details
4. Remove any meta-commentary or generic filler text
5. Ensure all sections focus specifically on {company}'s protocol mechanics and ecosystem
...
Ensure the report is comprehensive, accurate, and technically focused. References should be included at the end if present in the original report."""
        ...
        try:
            response = await self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert DeFi protocol analyst who refines blockchain research reports to be accurate, comprehensive, and technically precise."
                    },
                    {
                        "role": "user",
                        "content": prompt
